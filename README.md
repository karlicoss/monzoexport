...
For multiple exports, pass sources from oldest to newest, e.g. `DAL(["older.json", "newer.json"])`.
For duplicate transaction IDs, the last record wins; transactions retain the order in which their IDs first appeared.
